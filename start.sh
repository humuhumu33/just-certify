...
#   1. Initialize Kubo at IPFS_PATH if this is a fresh volume.
#   2. Launch the Kubo daemon in the background.
#   3. Wait for its HTTP RPC to respond.
#   4. Exec the service (replaces shell → signals flow via tini).
...
# 2. Kubo daemon in the background.
echo "start: launching Kubo daemon"
ipfs daemon --routing=dhtclient >/var/log/ipfs.log 2>&1 &
KUBO_PID=$!

# 3. Wait for RPC (up to 60s) before starting the service.
...
# 4. Hand control to the service. `exec` makes it PID 1 under tini,
#    so SIGTERM from Fly / docker stop propagates cleanly and the
#    service's fail-fast `/v1/health`-ping gate runs against the
#    now-ready Kubo.
